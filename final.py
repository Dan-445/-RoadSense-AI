...
PROCESSED_FRAMES_DIR = r"output19/processed_frames"  # Directory to save processed frames (with bounding boxes)
OUTPUT_CSV_PATH = r"output19/VRI_detection_data.csv"  # Path to save the CSV output
OUTPUT_EXCEL_PATH = r"output19/VRI_detection_data.xlsx"  # Path to save the Excel output
OUTPUT_VIDEO_PATH = r"output19/damaged_video.mp4"  # Path to save the MP4 video output
...
# YOLO Detection and Processing Function with GPU support
def process_frames_with_yolo(frames_path, processed_frames_dir, model_path, output_excel_path, output_csv_path, gnss_ts_dict, output_video_path):
    # Load the YOLO model with GPU support if available
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = YOLO(model_path).to(device)  # Move the model to GPU if available
    ...
    for frame_file in sorted(os.listdir(frames_path)):
        if frame_file.endswith(".png"):
            ...
            # Move the frame to GPU for inference
            frame_tensor = torch.from_numpy(frame).to(device).float().permute(2, 0, 1).unsqueeze(0)
