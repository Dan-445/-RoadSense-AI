import cv2
import os
import numpy as np  # Import NumPy for handling depth maps
...
SVO_FILE_PATH = r"ZED_43.svo2"  # Path to your SVO2 file
GNSS_FILE_PATH = r"gnns/GNSS_43.json"  # Path to your GNSS JSON file
EXTRACTED_FRAMES_DIR = r"extracted_frame434"  # Directory where frames will be saved
PROCESSED_FRAMES_DIR = r"output/processed_frames434"  # Directory to save processed frames (with bounding boxes)
OUTPUT_CSV_PATH = r"output/43VRI_detection_data.csv"  # Path to save the CSV output
OUTPUT_EXCEL_PATH = r"output/43VRI_detection_data.xlsx"  # Path to save the Excel output
OUTPUT_VIDEO_PATH = r"output/43damaged_video.mp4"  # Path to save the MP4 video output
...
def extract_frames_from_svo(svo_file_path, frames_dir):
    ...
    Also saves the corresponding depth maps as .npy files.
    """
    zed = sl.Camera()
    ...
    # Loop through the SVO file and extract both left frames and depth data
    while zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
        zed.retrieve_image(image, sl.VIEW.LEFT)  # Retrieve only the left view
        zed.retrieve_measure(depth, sl.MEASURE.DEPTH)  # Retrieve depth map

        frame = image.get_data()
        depth_data = depth.get_data()  # Depth map for the frame

        frame_file = os.path.join(frames_dir, f"frame_{frame_count:06d}.png")
        depth_file = os.path.join(frames_dir, f"frame_{frame_count:06d}.npy")  # Ensure depth file is named correctly
        
        cv2.imwrite(frame_file, frame)  # Save the frame as PNG
        np.save(depth_file, depth_data)  # Save depth data as a .npy file
